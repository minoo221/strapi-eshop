  ...
  <footer class="mt-24 pt-8 pb-8 bg-gray-200">
    ...
    <div class="container mt-6 flex flex-col-reverse md:flex-row justify-between gap-6 md:gap-0">
      ...
      <div class="flex flex-col gap-2">
        ...
        <p class="text-gray-600">Copyright {{ new Date().getFullYear() }}</p>
      </div>
      <!-- Site pages -->
      <ul class="flex flex-col gap-2">
        <li v-for="page in pages" :key="page.path">
          <g-link :to="page.path" class="py-1">
            {{ page.title }}
          </g-link>
        </li>
      </ul>
      ...
      <ul class="flex flex-col gap-1 md:-mt-1">
        <li v-for="socialNetwork in $static.strapi.global.socialNetworks" :key="socialNetwork.id">
          <a :href="socialNetwork.url" target="_blank" rel="noopener noreferrer" class="py-1 flex flex-row items-center gap-2">
            <img :src="getStrapiMedia(socialNetwork.icon.url)" :alt="socialNetwork.title" class="w-4" />
